...
//信息完善积分规则
class ActivePerfectingBox extends Component {
    constructor(props) {
        super(props);
        this.state = {
            loading: false,
    ...
    loadData(page, size, searchData) {
        ...
        data.type = 4;//规则类型 1：消费积分 2：充值积分 3：活跃积分-新用户注册 4：活跃积分-个人信息完善
        ...
        if (!this.state.submitLoading) {
            ...
            this.props.form.validateFieldsAndScroll((err, fieldsValue) => {
                if (!err) {
                    ...
                    let data = {
                        "type": 4,//规则类型 1：消费积分 2：充值积分 3：活跃积分-新用户注册 4：活跃积分-个人信息完善
    ...
    render() {
        ...
        const columns = [{
            ...
            className: 'column-content-inspection',
            render: (row) => {
                return <div>信息完善即送积分{row.memberScore}分</div>
            }
        }, {
            title: '生效状态',
                ...
                <div style={{height: "32px", marginBottom: "20px"}}>
                    {window.getPerValue('vipAddEditRule') ?
                        <Button type="primary" className="vip-search-button" onClick={this.addRule.bind(this)}><Icon
                            type="plus"/>新建信息完善积分规则</Button> : null}
                ...
                >
                    <Form style={{padding: "24px"}}>
                        ...
                        <FormItem label="换算规则" {...formModalLayout} require={true}>
                            <div className="vip-points-flex">信息完善即送积分
                                {getFieldDecorator(`memberScore`, {
                                    ...
                                    <Input style={{margin: "0 8px"}} placeholder="请输入"/>
                                )}
                                分
                            </div>
                        </FormItem>
...
export default Form.create()(ActivePerfectingBox);