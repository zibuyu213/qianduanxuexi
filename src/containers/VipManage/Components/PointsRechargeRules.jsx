...
//充值积分规则
class PointsRechargeRules extends Component {
    constructor(props) {
        super(props);
        this.state = {
            loading: false,
    ...
    componentWillReceiveProps(nextProps) {
        ...
        if (oldGradeList !== newGradeList && newGradeList.length > 0) {
            let list = [];
    ...
    loadData(page, size, searchData) {
        ...
        data.type = 2;//规则类型 1：消费积分 2：充值积分 3：活跃积分-新用户注册 4：活跃积分-个人信息完善
        ...
        if (!this.state.submitLoading) {
            ...
            this.props.form.validateFieldsAndScroll((err, fieldsValue) => {
                if (!err) {
                    ...
                    const rateArray = [];
                    fieldsValue['rateList'].forEach(item => {
                        rateArray.push({
                    ...
                    let data = {
                        "type": 2,//规则类型 1：消费积分 2：充值积分 3：活跃积分-新用户注册 4：活跃积分-个人信息完善
    ...
    render() {
        ...
        const columns = [{
            ...
            className: 'column-content-inspection',
            render: (row) => {
                return <div>每充值{row.minAmount}元，获得积分{row.memberScore}分</div>
            }
        }, {
            title: '会员等级及积分倍率',
            className: "column-content-inspection",
                ...
                <div style={{ height: "32px", marginBottom: "20px" }}>
                    {window.getPerValue('vipAddEditRule') ?
                        <Button type="primary" className="vip-search-button" onClick={this.addRule.bind(this)}><Icon
                            type="plus" />新建充值积分规则</Button> : null}
                ...
                >
                    <Form style={{ padding: "24px" }}>
                        ...
                        <FormItem
                            ...
                            <div className="vip-points-flex">每充值
...
export default Form.create()(PointsRechargeRules);
